...
//			}
			...
			if (!Dialogs.showConfirmDialog("Set path to weka.jar", "Cannot find weka.jar.\n\nDo you want to select it manually from your Weka installation?"))
				return;
...
//			}
	...
	private void saveWekaData() {
		if (panel.getSelectedFeatures().isEmpty()) {
			Dialogs.showErrorMessage("Weka export", "No features selected for export!");
			return;
		}
		try {
			...
			Dialogs.showErrorNotification("Export for Weka", e);
		}
	}
